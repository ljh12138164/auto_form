  ...
  "formConfig": [
    {
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" @click="validateAndImportJson" :disabled="!jsonInput.trim()">
            验证并导入
          </el-button>
        </div>
      </template>
    ...
    <div class="form-preview" v-if="selectedForm">
      <el-card>
        ...
        <div class="preview-content">
          <h3>{{ selectedForm.title || selectedForm.formName || '表单预览' }}</h3>
...
const selectedForm = computed(() => {
  if (selectedFormIndex.value !== null && formList.value[selectedFormIndex.value]) {
...
const generateVueCode = (formData: any) => {
  ...
  const generateFieldTemplate = (field: any) => {
    ...
    const required = field.required ? ` :rules="[{ required: true, message: '请输入${field.label}', trigger: 'blur' }]"` : ''
    ...
    switch (field.type) {
      ...
      case 'radio':
        const radioOptions = field.options?.map((option: any) => 
          VueTemplates.optionTemplates.radio(option)
        ).join('\n') || ''
        return VueTemplates.fieldTemplates.radio(field, required, radioOptions, fieldName)
      
      case 'checkbox':
        const checkboxOptions = field.options?.map((option: any) => 
          VueTemplates.optionTemplates.checkbox(option)
        ).join('\n') || ''
        return VueTemplates.fieldTemplates.checkbox(field, checkboxOptions, fieldName)
  ...
  return VueTemplates.mainTemplate(componentClassName, formName, fieldsTemplate, formDataTemplate)
}
...
const downloadCode = () => {
  ...
  const formName = selectedForm.value.title || selectedForm.value.formName || 'DynamicForm'
...
const validateAndImportJson = () => {
  ...
  try {
    ...
    const newForm = {
      ...parsedData,
      title: parsedData.title || parsedData.formName || `导入的表单${formList.value.length + 1}`,
      id: Date.now(), // 添加唯一ID
